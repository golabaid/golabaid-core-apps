...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		if ($this->config->item('use_ci_email', 'email') && isset($email_config) && is_array($email_config))
		{
	...
	public function forgotten_password($identity)
	{
		if ($this->ion_auth_model->forgotten_password($identity))
		{
			...
			if ($user)
			{
				...
				if (!$this->config->item('use_ci_email', 'email'))
				{
				...
				{
					if ($this->email->prepare($this->config->item('site_title', 'ion_auth'),$this->config->item('admin_email', 'ion_auth'),$user->email,NULL,NULL,$this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'),$data,$this->config->item('email_forgot_password', 'email'),NULL))
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
					}
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			{
				if ($this->email->prepare($this->config->item('site_title', 'ion_auth'),$this->config->item('admin_email', 'ion_auth'),$profile->email,NULL,NULL,$this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_new_password_subject'),$data,$this->config->item('email_forgot_password_complete', 'email'),NULL))
				{
					$this->set_message('password_change_successful');
					$this->ion_auth_model->trigger_events(array('post_password_change', 'password_change_successful'));
					return TRUE;
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array())
	{
		...
		else
		{
			...
			{
				if ($this->email->prepare($this->config->item('site_title', 'ion_auth'),$this->config->item('admin_email', 'ion_auth'),$email,NULL,NULL,$this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_activation_subject'),$data,$this->config->item('email_activate', 'email'),NULL))
				{
					$this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
					$this->set_message('activation_email_successful');
					return $id;
